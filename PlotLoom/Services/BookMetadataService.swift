...
enum BookMetadataProvider: String, Sendable, Equatable, Hashable {
...
struct BookMetadataCandidate: Identifiable, Equatable, Hashable {
...
struct BookMetadataService: Sendable {
    ...
    func search(title: String, author: String) async throws -> [BookMetadataCandidate] {
        ...
        let openLibrary = try await searchOpenLibrary(title: trimmedTitle, author: author.trimmed)
        var candidates = await enrichOpenLibraryCandidates(openLibrary)

        if candidates.prefix(3).allSatisfy({ ($0.description ?? "").isEmpty }) {
            let google = (try? await searchGoogleBooks(title: trimmedTitle, author: author.trimmed)) ?? []
        ...
        return ranked(candidates, title: trimmedTitle, author: author.trimmed)
    }
