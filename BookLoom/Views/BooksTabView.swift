...
/// Surfaces Goodreads share-extension imports waiting for review. Tapping a
/// row opens the full import sheet for that URL; the trash button removes it
/// from the queue without opening it. Cancelling the import sheet keeps the
/// URL here so a user can return to it later instead of losing the share.
private struct ImportInboxBanner: View {
